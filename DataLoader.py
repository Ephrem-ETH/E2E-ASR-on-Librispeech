...
def data_processing(data, data_type="train"):
    ...
    for (waveform, _, utterance, speaker_id, utterance_id) in data:
        if data_type == 'train':
            ...
            # mfcc = torchaudio.functional.sliding_window_cmn(mfcc, center=True, norm_vars=True)
            # delta = torchaudio.functional.compute_deltas(mfcc, win_length=3)
            # delta2 = torchaudio.functional.compute_deltas(delta, win_length=3)
            # feature_vector = torch.cat([mfcc,delta,delta2]).transpose(0, 1)
